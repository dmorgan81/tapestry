...
static void prv_window_load(Window *window) {
    ...
    s_time_layer = time_layer_create(GRect(5, PBL_IF_RECT_ELSE(4, 8), rect.size.w - 10, rect.size.h - PBL_IF_RECT_ELSE(7, 11)), s_tapestry_layer);

    h = bounds.size.h / 4;
    s_banner_layer = banner_layer_create(GRect(PBL_IF_RECT_ELSE(5, 30), bounds.size.h - h, bounds.size.w - PBL_IF_RECT_ELSE(10, 60), h - 10));
#ifdef PBL_ROUND
    GRect frame = layer_get_frame(s_banner_layer);
    frame.origin.y -= 10;
