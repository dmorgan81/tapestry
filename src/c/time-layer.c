...
static void prv_update_proc(Layer *this, GContext *ctx) {
    ...
    FPoint origin = g2fpoint(frame.origin);

    FContext fctx;
    fctx_init_context(&fctx, ctx);
    fctx_set_offset(&fctx, fpoint_add(origin, FPointI(frame.size.w, 0)));
    ...
    fctx_set_offset(&fctx, fpoint_add(origin, FPointI(frame.size.w, frame.size.h)));
